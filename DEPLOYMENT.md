...
before the web server starts** (`scripts/docker-entrypoint.sh`). That is
deliberate: "pre-deploy" hooks are a paid feature on most hosts, and a
container should be able to bring itself up unaided. Both steps are idempotent,
so restarts and redeploys are safe.

If `DATABASE_URL` is missing or still a placeholder, the container exits
immediately with a message saying so, rather than starting and serving 500s.
...
5. Migrations, settings, categories and the admin are applied by the
   container on boot — no paid pre-deploy hook needed.
...
window. Fine for getting the site up; move to a paid instance (or Neon's free
tier, which does not expire) before real purchases — a money ledger must not
live somewhere that gets deleted.

**Free web services sleep.** After ~15 minutes idle the instance spins down and
the next request takes roughly 50 seconds to wake it. The bundled GitHub Action
pings every 15 minutes, which also keeps it warm.
...
paid feature, so a free GitHub Actions equivalent ships in
`.github/workflows/cron.yml` — add two repository secrets (`APP_URL`,
`CRON_SECRET` = your `AUTH_SECRET`) and it runs itself. Any external pinger
works too:
